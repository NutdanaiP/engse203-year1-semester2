...
class FileManager {
  ...
  async appendFile(fileName, content) {
    try {
      ...
      logger.success(`Appended content to: ${fileName}`);
      return true;
    } catch (error) {
  ...
  async searchFiles(keyword) {
    try {
      ...
      console.log('─'.repeat(50));
      if (results.length === 0) {
        logger.warning('No files found containing the keyword');
      } else {
  ...
  async getFileStats(fileName) {
    try {
      ...
      logger.info(`Statistics for '${fileName}':`);console.log('─'.repeat(50));
