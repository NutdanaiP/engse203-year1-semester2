...
exports.getAll = (req, res) => {
  try {
    ...
    // Challenge 1: Sorting
    if (req.query.sort) {
      const sortField = req.query.sort; // 'price', 'name', 'stock', etc.
...
exports.bulkUpdate = (req, res) => {
  try {
    ...
    ids.forEach(id => {
      const updatedProduct = Product.partialUpdate(id, processedUpdates);
      if (updatedProduct) {
    ...
    res.json({
      success: true,
      message: `Updated ${results.successful.length} of ${ids.length} products`,
      data: {
